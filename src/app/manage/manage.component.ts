...
public DOBMonth:any;
  ...
  constructor(private manageService: ManageService, private loginService: LoginService, private selectstaff: StaffService, private _toastr: ToastsManager, vRef: ViewContainerRef, private router: Router, private sharedService: SharedService, private _otherservice: OtherSettingsService) {
  ...
  getWaitListData(restarauntid) {
    ...
    this.manageService.getTruflUserList(restarauntid).subscribe((res: any) => {
  ...
  Ok() {
    if (this.isempty === 'empty') {
      this.manageService.postUpdateEmptyBookingStatus(this.emptybookingid).subscribe((res: any) => {
        this.getWaitListData(this.restarauntid);
      }, (err) => {
        if (err === 0) {
          this._toastr.error('network error')
     ...
     // this.isMessageEdit = !this.isMessageEdit
      ...
      this.manageService.sendmessage(this.acceptdata.TruflUserID).subscribe((res: any) => {
        if (res._Data[0].TruflUserID) {
          this.manageService.changeicon(this.restarauntid, this.acceptdata.BookingID, this.acceptdata.TruflUserID).subscribe((res: any) => {
           this.changeIconDataResponse = res;
            this.errorcode = res._ErrorCode;
            this.showDialog = !this.showDialog;

            if (this.errorcode === 0) {
              this.manageService.postPremiumUserdetails(this.acceptdata.TruflUserID, this.restarauntid, this.billamount, this.rewardtype).subscribe((res: any) => {
      ...
      this.manageService.sendmessage(this.acceptsidenavdata.TruflUserID).subscribe((res: any) => {
        if (res._Data[0].TruflUserID) {
          this.manageService.changeicon(this.restarauntid, this.acceptsidenavdata.BookingID, this.acceptsidenavdata.TruflUserID).subscribe((res: any) => {
            this.errorcode = res._ErrorCode;
            this.showDialog = !this.showDialog;
            if (this.errorcode === 0) {
              this.manageService.postPremiumUserdetails(this.acceptsidenavdata.TruflUserID, this.restarauntid, this.billamount, this.rewardtype).subscribe((res: any) => {
      ...
      this.manageService.sendmessage(this.notifydata.TruflUserID).subscribe((res: any) => {
        if (res._Data[0].TruflUserID) {
          this.manageService.changeiconpush(this.restarauntid, this.notifydata.BookingID).subscribe((res: any) => {
            this.getWaitListData(this.restarauntid);
            this.showDialog = !this.showDialog;
            if (res != null) {
              this.showProfile = false;
  ...
  sendMessage(message) {
    ...
    this.manageService.pushNotification(obj).subscribe((res: any) => {
      if (res == true) {
  ...
  acceptoffer(data) {
    ...
    this.manageService.setRowData(data);
  ...
  public getservers() {
    this.manageService.getservers(this.restID).subscribe((res: any) => {

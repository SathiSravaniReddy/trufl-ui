...
export class SeataguestService {
  ...
  getwaitlist(restID: any, openDate: any, SessionID:any) {
    return this.http.get(constant.truflAPI + constant.truflBase + 'WaitListUser/GetWaitListUsers/' + restID + '/' + openDate + '/' + SessionID).map(
      (res) => res.json()
    ).catch(this.handleError);
...
/*update getseated now*/
  ...
  GetServerwiseSnap(RestaurantID) {
    return this.http.get(constant.truflAPI + constant.truflBase + '/WaitListUser/GetServerwiseSnapshot/' + RestaurantID + '/Active').map(
          (res) => res.json()
      ).catch(this.handleError);
  }
